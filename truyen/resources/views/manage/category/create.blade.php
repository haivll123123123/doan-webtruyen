			...
			<div class="col-md-12">
				...
				<div class="card card-primary">
					<div class="card-header">
						<h3 class="card-title"><i>Thêm Thể Loại by</i> <strong>{{auth()->user()->name}}</strong></h3>
					</div>
					<!-- /.card-header -->
					<!-- form start -->
                    <form role="form" action="{{route('manage.category.store')}}" method="POST" id="quickForm">
                        @csrf
						<div class="card-body">
							<div class="form-group">
								<label for="#">Tên Thể Loại</label>
								<input type="text" name="name" class="form-control" id="#" placeholder="Nhập Tên Thể Loại">
                            </div>
                            <div class="help-info">
                                @if ($errors->has('name'))
